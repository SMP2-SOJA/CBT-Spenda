// CBT APP ENGINE - Copyright (c) 2026 Rahmat Hidayat H. | @spenda-digi
const API = ""; 
let activeUser = null; 
...
async function loadPublicData() {
    try {
        ...
        renderPublicTable();
    } catch(e) { console.log("Gagal memuat live score"); }
}
...
function renderPublicTable() {
    ...
    let html = filtered.map(a => {
        ...
        return `<tr class="hover:bg-slate-50"><td class="p-2 font-bold text-slate-800">${a.student_name}</td><td class="p-2 font-medium text-slate-600">${a.kelas||'-'} <br><span class="text-[9px]">${a.exam_name}</span></td><td class="p-2 text-center"><span class="px-2 py-1 rounded text-[9px] font-bold ${badge}">${a.status}</span></td><td class="p-2 text-center font-black text-sm text-blue-600">${nilaiTampil}</td></tr>`;
    }).join('');
    document.getElementById('public-table-body').innerHTML = html || `<tr><td colspan="4" class="text-center p-4 text-slate-400">Belum ada data.</td></tr>`;
}
...
function toggleAdminSidebar() { const s = document.getElementById('admin-sidebar'); const o = document.getElementById('admin-overlay'); if(s && o) { s.classList.toggle('-translate-x-full'); o.classList.toggle('hidden'); } }
...
function togglePass(inputId, iconId) { const p = document.getElementById(inputId); const i = document.getElementById(iconId); if(p.type === 'password') { p.type = 'text'; i.classList.replace('fa-eye-slash', 'fa-eye'); } else { p.type = 'password'; i.classList.replace('fa-eye', 'fa-eye-slash'); } }

document.addEventListener('keyup', (e) => { if (isExamActive && (e.key === 'PrintScreen' || e.keyCode === 44)) { navigator.clipboard.writeText(''); Swal.fire('Akses Dilarang!', 'Tangkapan layar diblokir!', 'warning'); } });
...
function deteksiKecurangan() {
    ...
    if (curangCount >= 3) { isExamActive = false; Swal.fire({title: 'TERKUNCI!', text: 'Anda keluar ujian lebih dari 3 kali!', icon: 'error', allowOutsideClick: false}).then(() => { submitUjian(false, true); }); } else { Swal.fire('PERINGATAN!', `Dilarang pindah aplikasi/tab! (${curangCount}/3)`, 'warning'); }
}
        ...
        if (data.status === "success") {
            ...
            if (activeUser.role.toLowerCase() === 'siswa') { document.getElementById('view-siswa-token').classList.remove('hidden'); document.getElementById('nama-siswa-welcome').innerText = activeUser.name; checkPendingSubmit(false); } 
                ...
                if (activeUser.role.toLowerCase() === 'guru') { 
                    ...
                    document.getElementById('role-badge').innerText = `GURU: ${activeUser.mapel || ''}`; 
                } 
...
async function mulaiUjian() {
    const pin = document.getElementById('input-pin').value; if(!pin) return Swal.fire('Error', 'Isi PIN!', 'warning'); Swal.fire({ title: 'Menyiapkan...', didOpen: () => Swal.showLoading() });
    try {
        ...
        if(data.status === "success") {
            ...
            if(!dataSoal.questions || dataSoal.questions.length === 0) return Swal.fire('Oops', 'Soal kosong!', 'error');
    ...
    } catch(e) { Swal.fire('Error', 'Gagal memuat token.', 'error'); }
}
...
function getLiveScore() {
    if(!cbtQuestions || cbtQuestions.length === 0) return 0; if(cbtQuestions[0].tipe === 'GFORM') return 'G-Form';
    ...
    cbtQuestions.forEach((q, index) => {
        ...
        else if (q.tipe === 'PGK') { kunciArr = (q.kunci||"").replace(/\s/g, '').toLowerCase().split(',').filter(x=>x); aArr = ans.replace(/\s/g, '').toLowerCase().split(',').filter(x=>x); if(kunciArr.length>0) { aArr.forEach(a => { if(kunciArr.includes(a)) betul++; }); totalGet += (betul / kunciArr.length) * bobot; } } 
...
function renderCbtGrid() {
    ...
    for(let i = 0; i < cbtQuestions.length; i++) {
        ...
        if (cbtAnswers[i].ans !== "") { if (cbtQuestions[i].tipe === 'BS' || cbtQuestions[i].tipe === 'TS' || cbtQuestions[i].tipe === 'SIFAT') { isAnswered = cbtAnswers[i].ans.indexOf('-') === -1; } else { isAnswered = true; } }
        ...
        html += `<button onclick="showCbtQuestion(${i}); if(window.innerWidth < 768) toggleCbtNav();" class="w-full aspect-square flex items-center justify-center rounded-lg font-black text-xs border-2 transition transform active:scale-95 ${statusClass} ${activeRing}">${i+1}</button>`;
...
function showCbtQuestion(index) {
    ...
    if(imgUrl && imgUrl.includes('drive.google.com')) { 
        ...
        if(idMatch && idMatch[1]) { imgUrl = `https://drive.google.com/uc?export=view&id=${idMatch[1]}`; } 
    }
    if (imgUrl && q.tipe !== 'GFORM' && imgUrl.startsWith('http')) { 
        divMedia.innerHTML = `<img src="${imgUrl}" class="w-auto max-w-full h-auto max-h-[50vh] rounded-xl border mx-auto mb-3 object-contain">`; 
    }
    ...
    if(q.tipe === 'GFORM') { divOpsi.innerHTML = `<div class="gform-container"><iframe src="${q.tanya || q.gform_url}"></iframe></div>`; cbtSaveAnswer("COMPLETED"); } 
        ...
        if (q.tipe === 'JODOH') {
            ...
            htmlOpsi += `<div class="bg-blue-50 p-3 rounded-xl border border-blue-200 mb-3"><p class="text-[10px] font-bold text-blue-800 mb-2">JODOHKAN PERNYATAAN:</p><div class="space-y-2">`;
            for(let i=0; i<totalPairs; i++) { let currentSaved = savedArr[i] ? savedArr[i].replace(/[0-9]/g, '') : ''; let labelText = premises[i] ? premises[i] : `Pasangan No. ${i+1}`; htmlOpsi += `<div class="flex flex-col md:flex-row items-start md:items-center justify-between p-2 bg-white border rounded-lg gap-2"><span class="font-bold text-slate-700 text-xs md:w-1/2 leading-snug">${formatMath(labelText)}</span><select class="jodoh-select p-2 border rounded-lg text-xs font-bold bg-emerald-50 text-emerald-800 w-full md:w-1/2" onchange="cbtSaveJodoh(${totalPairs})"><option value="">- Pilih Jawaban -</option>`; opsiArray.forEach((val, idx) => { let huruf = abjad[idx]; let isSel = (currentSaved === huruf) ? "selected" : ""; htmlOpsi += `<option value="${huruf}" ${isSel}>${formatMath(val)}</option>`; }); htmlOpsi += `</select></div>`; } htmlOpsi += `</div></div>`;
        } else {
            divTanya.innerHTML = formatMath(q.tanya || "");
            if (q.tipe === 'PG') { htmlOpsi += `<div class="space-y-2">`; opsiArray.forEach((val, idx) => { let huruf = abjad[idx] || ''; let isChecked = (savedAns === huruf) ? "checked" : ""; htmlOpsi += `<label class="flex items-center p-2 border rounded-xl cursor-pointer bg-white transition hover:border-blue-300"><input type="radio" name="cbt_ans" value="${huruf}" ${isChecked} onchange="cbtSaveAnswer(this.value)" class="w-4 h-4 mr-3 accent-blue-600"><span class="font-black text-xs mr-3 bg-slate-100 border w-6 h-6 flex items-center justify-center rounded-full">${huruf}</span><span class="text-xs font-medium text-slate-700 leading-snug">${formatMath(val)}</span></label>`; }); htmlOpsi += `</div>`; }
            else if (q.tipe === 'PGK') { let savedArr = savedAns ? savedAns.split(',') : []; htmlOpsi += `<div class="space-y-2">`; opsiArray.forEach((val, idx) => { let huruf = abjad[idx] || ''; let isChecked = savedArr.includes(huruf) ? "checked" : ""; htmlOpsi += `<label class="flex items-center p-2 border rounded-xl cursor-pointer bg-white transition hover:border-purple-300"><input type="checkbox" value="${huruf}" ${isChecked} onchange="cbtSaveCheckbox()" class="cbt-pgk-cb w-5 h-5 mr-3 accent-purple-600 rounded"><span class="text-xs font-medium text-slate-700 leading-snug">${formatMath(val)}</span></label>`; }); htmlOpsi += `</div>`; }
            else if (q.tipe === 'BS' || q.tipe === 'TS' || q.tipe === 'SIFAT') {
                let savedArr = savedAns ? savedAns.split(',') : []; let headers = q.tipe === 'BS' ? ['Benar', 'Salah'] : (q.tipe === 'TS' ? ['Sesuai', 'Tidak Sesuai'] : ['Komutatif', 'Asosiatif', 'Distributif']);
                let statements = opsiArray.length > 0 ? opsiArray : ['Pernyataan 1', 'Pernyataan 2'];
                htmlOpsi += `<div class="overflow-x-auto rounded-xl border shadow-sm"><table class="w-full text-xs text-left"><thead class="bg-slate-100 text-slate-600"><tr><th class="p-3 border-b">Pernyataan</th>`;
                headers.forEach(h => htmlOpsi += `<th class="p-3 border-b text-center font-bold min-w-[60px]">${formatMath(h)}</th>`); htmlOpsi += `</tr></thead><tbody class="divide-y bg-white">`;
                statements.forEach((val, idx) => { htmlOpsi += `<tr><td class="p-3 text-slate-700 font-medium leading-snug">${formatMath(val)}</td>`; headers.forEach((h, hIdx) => { let huruf = abjad[hIdx]; let isChecked = savedArr[idx] === huruf ? 'checked' : ''; htmlOpsi += `<td class="p-3 text-center border-l"><input type="radio" name="matrix_${idx}" value="${huruf}" ${isChecked} onchange="cbtSaveMatrix(${statements.length})" class="w-4 h-4 accent-blue-600 cursor-pointer"></td>`; }); htmlOpsi += `</tr>`; }); htmlOpsi += `</tbody></table></div>`;
            }
            else if (q.tipe === 'ISIAN') { htmlOpsi += `<input type="text" onkeyup="cbtSaveAnswer(this.value)" value="${savedAns}" class="w-full p-3 border rounded-xl text-xs font-bold outline-none bg-white focus:border-blue-500" placeholder="Ketik jawaban singkat...">`; }
            else if (q.tipe === 'ESAI') { htmlOpsi += `<textarea onkeyup="cbtSaveAnswer(this.value)" class="w-full p-3 border rounded-xl h-24 text-xs outline-none bg-white focus:border-blue-500" placeholder="Uraikan jawaban Anda...">${savedAns}</textarea>`; }
...
async function checkPendingSubmit(showFeedback = false) {
    ...
    if(pendingData) {
        ...
        if(navigator.onLine) {
            ...
            try {
                await fetch(API + '/siswa/submit', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: pendingData });
            ...
            } catch(e) { if(showFeedback) Swal.fire('Gagal', 'Server sibuk.', 'error'); }
        }
    }
}
...
async function submitUjian(showConfirm = true, isForceCurang = false) {
    let benar = 0; let salah = 0; let detail = []; let isGformOnly = cbtQuestions.length > 0 && cbtQuestions[0].tipe === 'GFORM'; let nilaiAkhir = 0; let totalMaksimal = 0; let totalDiperoleh = 0;
    if(!isGformOnly) {
        cbtQuestions.forEach((q, index) => {
            let ans = cbtAnswers[index].ans; let status = 'Salah'; let bobot = q.skor ? parseFloat(q.skor) : 1; let poin = 0;
            if (q.kunci && q.kunci.trim() === '') { status = 'Koreksi Guru'; poin = 0; }
            else if (q.tipe === 'PG') { totalMaksimal += bobot; if((ans||"").trim().toLowerCase() === (q.kunci||"").trim().toLowerCase()) { status = 'Benar'; poin = bobot; totalDiperoleh += bobot; benar++; } else salah++; } 
            else if (q.tipe === 'PGK') { totalMaksimal += bobot; let kunciArr = (q.kunci||"").toLowerCase().split(',').filter(x=>x); let ansArr = (ans||"").toLowerCase().split(',').filter(x=>x); if(kunciArr.length > 0) { let betul = 0; ansArr.forEach(a => { if(kunciArr.includes(a)) betul++; }); if(betul === kunciArr.length && ansArr.length === kunciArr.length) { status = 'Benar'; poin = bobot; totalDiperoleh += bobot; benar++; } else if(betul > 0) { status = `Sebagian (${betul}/${kunciArr.length})`; poin = (betul / kunciArr.length) * bobot; totalDiperoleh += poin; benar++; } else salah++; } else salah++; } 
            else if (q.tipe === 'BS' || q.tipe === 'TS' || q.tipe === 'SIFAT') { totalMaksimal += bobot; let kArr = (q.kunci||"").toUpperCase().split(',').filter(x=>x); let aArr = (ans||"").toUpperCase().split(','); let correctCount = 0; for(let j=0; j<kArr.length; j++) { if(aArr[j] === kArr[j] && aArr[j] !== '-' && aArr[j] !== "") correctCount++; } if(correctCount === kArr.length && kArr.length > 0) { status = 'Benar'; poin = bobot; totalDiperoleh += bobot; benar++; } else if (correctCount > 0) { status = `Sebagian (${correctCount}/${kArr.length})`; poin = (correctCount / kArr.length) * bobot; totalDiperoleh += poin; benar++; } else salah++; }
            else if (q.tipe === 'JODOH') { totalMaksimal += bobot; let kArr = (q.kunci||"").toLowerCase().split(',').filter(x=>x); let aArr = (ans||"").toLowerCase().split(',').filter(x=>x); let correctCount = 0; kArr.forEach(k => { if(aArr.includes(k)) correctCount++; }); if(correctCount === kArr.length && kArr.length > 0) { status = 'Benar'; poin = bobot; totalDiperoleh += bobot; benar++; } else if (correctCount > 0) { status = `Sebagian (${correctCount}/${kArr.length})`; poin = (correctCount / kArr.length) * bobot; totalDiperoleh += poin; benar++; } else salah++; }
            else if (q.tipe === 'ISIAN' || q.tipe === 'ESAI') { let kWords = (q.kunci || "").toLowerCase().match(/[a-z0-9]+/gi) || []; let aWords = (ans || "").toLowerCase().match(/[a-z0-9]+/gi) || []; if (kWords.length > 0) { totalMaksimal += bobot; let matchWords = 0; let aWordsUnique = [...new Set(aWords)]; kWords.forEach(kw => { if(aWordsUnique.includes(kw)) matchWords++; }); if(matchWords === kWords.length) { status = 'Benar'; poin = bobot; totalDiperoleh += bobot; benar++; } else if(matchWords > 0) { status = `Sebagian (${matchWords}/${kWords.length})`; poin = (matchWords / kWords.length) * bobot; totalDiperoleh += poin; benar++; } else salah++; } else { status = 'Koreksi Guru'; poin = 0; } }
        ...
        }); nilaiAkhir = totalMaksimal > 0 ? Math.round((totalDiperoleh / totalMaksimal) * 100) : 0;
    ...
    const sendLogic = async () => {
        ...
        if(!navigator.onLine) { localStorage.setItem(`pending_submit_${activeUser.username}`, JSON.stringify(payload)); Swal.fire('Offline Saved', 'Jawaban tersimpan aman di HP.', 'warning'); return; }
        ...
        } catch(e) { localStorage.setItem(`pending_submit_${activeUser.username}`, JSON.stringify(payload)); Swal.fire('Error', 'Disimpan offline.', 'warning'); }
    };
    if (showConfirm && !isForceCurang) { Swal.fire({ title: 'Kumpulkan Ujian?', text: "Apakah yakin ingin menyudahi?", icon: 'warning', showCancelButton: true }).then((r) => { if(r.isConfirmed) sendLogic(); }); } else sendLogic();
}

function showPage(p) { 
    document.querySelectorAll('.page-content').forEach(el => el.classList.add('hidden')); 
    const elPage = document.getElementById('page-'+p); if (elPage) elPage.classList.remove('hidden'); 
    const pageTitle = document.getElementById('page-title'); if (pageTitle) pageTitle.innerText = p.toUpperCase();
...
function renderActivityTable() {
    ...
    let blocked = filtered.filter(a => a.status && a.status.includes('Curang')); let finished = filtered.filter(a => a.status === 'Selesai'); let working = filtered.filter(a => a.status === 'Mengerjakan' || (!a.status.includes('Curang') && a.status !== 'Selesai'));
    ...
    const buildRows = (arr, isWorking, badge) => arr.map(a => {
        let btn = ''; if(a.status && a.status.includes('Curang')) btn = `<br><button onclick="resetSiswa('${a.student_name}', '${a.exam_name}')" class="mt-1 bg-blue-600 text-white px-2 py-0.5 rounded text-[8px]"><i class="fa fa-unlock"></i> Buka Kunci</button>`;
...
async function resetSiswa(nama, mapel) { Swal.fire({ title: 'Buka Akses?', icon: 'warning', showCancelButton: true }).then(async (r) => { if(r.isConfirmed) { await fetch(API + '/admin/reset-siswa', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({student_name: nama, mapel: mapel}) }); loadStats(); } }); }
async function saveJadwal() {
    ...
    if(!mapel || !tanggal || !jam || !durasi) return Swal.fire('Oops','Lengkapi jadwal!','warning');
    ...
    loadJadwal(); Swal.fire('Sukses','Jadwal diaktifkan!','success');
}
...
async function importExcelSoal() {
    ...
    if(!file || !exam_id) return Swal.fire('Oops', 'Lengkapi kolom!', 'warning');
...
function tambahBarisSoal() {
    ...
    const html = `<div class="question-item bg-slate-50 p-4 rounded-xl border relative mt-4" data-no="${questionCount}"><div class="absolute -left-2 -top-2 w-6 h-6 bg-slate-800 text-white rounded-full flex items-center justify-center font-black text-xs">${questionCount}</div><button onclick="this.parentElement.remove()" class="absolute -right-2 -top-2 w-5 h-5 bg-red-500 text-white rounded-full text-xs"><i class="fa fa-times"></i></button><div class="grid grid-cols-1 gap-2"><div class="grid grid-cols-3 gap-2"><select class="q-tipe p-2 border rounded text-xs"><option value="PG">PG Biasa</option><option value="PGK">PG Kompleks</option><option value="JODOH">Menjodohkan</option><option value="ISIAN">Isian Singkat</option><option value="ESAI">Uraian / Esai</option><option value="BS">Benar/Salah</option><option value="TS">Tabel Sesuai</option><option value="SIFAT">Sifat Komutatif</option></select><input type="text" class="q-kunci p-2 border rounded text-xs" placeholder="Kunci"><input type="number" class="q-skor p-2 border rounded text-xs" value="1"></div><input type="text" class="q-image p-2 border rounded text-xs" placeholder="Link Gambar"><textarea class="q-tanya p-2 border rounded text-xs h-12" placeholder="Pertanyaan"></textarea><input type="text" class="q-opsi p-2 border rounded text-xs" placeholder="Opsi A ||| Opsi B"></div></div>`;
...
async function loadBankSoal() {
    ...
    document.getElementById('banksoal-container').innerHTML = Object.entries(groups).map(([examId, list]) => {
        let rows = list.map(q => `<tr class="border-b"><td class="p-2 text-xs"><span class="bg-blue-100 text-blue-700 px-2 py-0.5 rounded font-black text-[9px]">${q.tipe}</span></td><td class="p-2 text-xs font-medium">${formatMath(q.tanya).substring(0,70)}... <br><span class="text-[9px] text-emerald-600 font-bold">Kunci: ${q.kunci}</span></td><td><button onclick="hapusSoal(${q.id})" class="text-red-500 text-xs"><i class="fa fa-trash"></i></button></td></tr>`).join('');
...
function renderNilaiTable() {
    ...
    document.getElementById('nilai-body').innerHTML = window.filteredResultsData.map(n => {
        let statusKKMHtml = '-'; if (!isNaN(valKKM)) { statusKKMHtml = (parseFloat(n.nilai) || 0) >= valKKM ? `<span class="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded text-[10px] font-bold">TUNTAS</span>` : `<span class="bg-red-100 text-red-700 px-2 py-0.5 rounded text-[10px] font-bold">REMEDIAL</span>`; }
        return `<tr><td class="p-2 font-medium">${n.student_name}<br><span class="text-[9px] text-slate-400">Kls: ${n.kelas}</span></td><td>${n.mapel}</td><td>${n.tanggal || '-'}</td><td class="text-emerald-600 font-bold">${n.benar || 0}</td><td class="text-red-500 font-bold">${n.salah || 0}</td><td class="font-black">${n.nilai}</td><td>${statusKKMHtml}</td><td><button onclick='lihatDetail(${JSON.stringify(n.detail_jawaban || "[]")})' class="bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-[10px]">Detail</button></td></tr>`;
    }).join('');
}

function lihatDetail(detailJson) {
    let details = []; try { details = typeof detailJson === 'string' ? JSON.parse(detailJson) : detailJson; } catch(e){}
    if(details.length === 0) return Swal.fire('Info', 'Detail analisis tidak tersedia.', 'info');
    document.getElementById('detail-content').innerHTML = details.map(d => {
        let colored = colorizeAnswer(d.jawab, d.kunci, d.tipe); let badgeColor = d.status.includes('Benar') ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700';
...
function exportExcelDetail() {
    ...
    window.filteredResultsData.forEach(r => {
        ...
        if(isKKMActive) { let stText = (parseFloat(r.nilai) || 0) >= valKKM ? 'TUNTAS' : 'REMEDIAL'; let stCol = (parseFloat(r.nilai) || 0) >= valKKM ? '#047857' : '#dc2626'; tableHtml += `<td style="font-weight:bold; color:${stCol};">${stText}</td>`; }
