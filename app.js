...
function formatMath(text) {
    ...
    text = text.replace(/\^\((.*?)\)/g, '<sup>$1</sup>').replace(/\^([a-zA-Z0-9]+)/g, '<sup>$1</sup>').replace(/_\((.*?)\)/g, '<sub>$1</sub>').replace(/_([a-zA-Z0-9]+)/g, '<sub>$1</sub>');
    return text;
}
...
document.addEventListener('keyup', (e) => { if (isExamActive && (e.key === 'PrintScreen' || e.keyCode === 44)) { navigator.clipboard.writeText(''); Swal.fire('Akses Dilarang!', 'Tangkapan layar (Screenshot) terdeteksi dan tidak diizinkan!', 'warning'); } });
...
function deteksiKecurangan() {
    ...
    if (curangCount >= 3) { isExamActive = false; Swal.fire({title: 'UJIAN DIBATALKAN!', text: 'Anda terdeteksi keluar dari layar ujian lebih dari 3 kali. Ujian otomatis dikunci!', icon: 'error', allowOutsideClick: false}).then(() => { submitUjian(false, true); }); } else { Swal.fire('PERINGATAN KECURANGAN!', `Sistem mendeteksi Anda meminimalkan layar/membuka tab lain! DILARANG KERAS KELUAR DARI HALAMAN UJIAN! (Peringatan ${curangCount}/3)`, 'warning'); }
}
...
function getLiveScore() {
    ...
    cbtQuestions.forEach((q, index) => {
        ...
        if (q.kunci && q.kunci.trim() === '') {
            ...
            if(ansBersih && ansBersih === kunciBersih) { totalSkorDiperoleh += bobot; } 
        } 
        else if (q.tipe === 'PGK') { 
            totalSkorMaksimal += bobot; let kunciArr = (q.kunci||"").replace(/\s/g, '').toLowerCase().split(',').filter(x=>x); let ansArr = (ans||"").replace(/\s/g, '').toLowerCase().split(',').filter(x=>x); 
            if(kunciArr.length > 0) { let betul = 0; ansArr.forEach(a => { if(kunciArr.includes(a)) betul++; }); if(betul === kunciArr.length && ansArr.length === kunciArr.length) { totalSkorDiperoleh += bobot; } else if(betul > 0) { totalSkorDiperoleh += (betul / kunciArr.length) * bobot; } } 
        } 
        else if (q.tipe === 'BS' || q.tipe === 'TS' || q.tipe === 'SIFAT') { 
            totalSkorMaksimal += bobot; let kArr = (q.kunci||"").replace(/\s/g, '').toUpperCase().split(',').filter(x=>x); 
            if ((q.tipe === 'BS' || q.tipe === 'TS') && kArr.some(k => k === 'S')) { kArr = kArr.map(k => k === 'B' ? 'A' : (k === 'S' ? 'B' : k)); } 
            let aArr = (ans||"").replace(/\s/g, '').toUpperCase().split(','); let correctCount = 0; let totalItems = kArr.length; 
            for(let j=0; j<totalItems; j++) { if(aArr[j] === kArr[j] && aArr[j] !== '-' && aArr[j] !== "") { correctCount++; } } 
            if(correctCount === totalItems && totalItems > 0) { totalSkorDiperoleh += bobot; } else if (correctCount > 0) { totalSkorDiperoleh += (correctCount / totalItems) * bobot; } 
        }
        else if (q.tipe === 'JODOH') { 
            totalSkorMaksimal += bobot; let kArr = (q.kunci||"").replace(/\s/g, '').toLowerCase().split(',').filter(x=>x); let aArr = (ans||"").replace(/\s/g, '').toLowerCase().split(',').filter(x=>x); 
            let correctCount = 0; let totalItems = kArr.length; kArr.forEach(k => { if(aArr.includes(k)) correctCount++; }); 
            if(correctCount === totalItems && totalItems > 0) { totalSkorDiperoleh += bobot; } else if (correctCount > 0) { totalSkorDiperoleh += (correctCount / totalItems) * bobot; } 
        }
        else if (q.tipe === 'ISIAN' || q.tipe === 'ESAI') { 
            let kWords = (q.kunci || "").toLowerCase().match(/[a-z0-9]+/gi) || []; let aWords = (ans || "").toLowerCase().match(/[a-z0-9]+/gi) || []; 
            if (kWords.length > 0) { totalSkorMaksimal += bobot; let matchWords = 0; let aWordsUnique = [...new Set(aWords)]; kWords.forEach(kw => { if(aWordsUnique.includes(kw)) matchWords++; }); 
            if(matchWords === kWords.length) { totalSkorDiperoleh += bobot; } else if(matchWords > 0) { totalSkorDiperoleh += (matchWords / kWords.length) * bobot; } } 
        }
    });
...
setInterval(() => {
    if (isExamActive && navigator.onLine) {
        ...
        if (start) { 
            let diffMs = Date.now() - parseInt(start); 
            dMins = Math.floor(diffMs / 60000); 
            dSecs = Math.floor((diffMs % 60000) / 1000); 
        }
        
        let terjawab = 0; let totalSoal = cbtQuestions.length;
        for(let i=0; i<totalSoal; i++){
            let a = cbtAnswers[i]?.ans || ""; let q = cbtQuestions[i];
            if(a === "") continue;
        ...
        let durasiTeks = `${dMins}m ${dSecs}s | ${terjawab}/${totalSoal} Soal`; 
        let lScore = getLiveScore(); 
    ...
    if(q.tipe === 'GFORM') { divOpsi.innerHTML = `<div class="gform-container"><iframe src="${q.tanya || q.gform_url}"></iframe></div>`; cbtSaveAnswer("COMPLETED"); 
        ...
        if (q.tipe === 'JODOH') {
            ...
            if (q.tipe === 'PG') { htmlOpsi += `<div class="space-y-2 md:space-y-3">`; opsiArray.forEach((val, idx) => { let huruf = abjad[idx] || ''; let isChecked = (savedAns === huruf) ? "checked" : ""; htmlOpsi += `<label class="flex items-center p-2 md:p-3 border-2 rounded-xl cursor-pointer bg-white transition hover:border-blue-300"><input type="radio" name="cbt_ans" value="${huruf}" ${isChecked} onchange="cbtSaveAnswer(this.value)" class="w-4 h-4 md:w-5 md:h-5 text-blue-600 mr-2 md:mr-3 accent-blue-600"><span class="font-black text-[10px] md:text-sm mr-2 md:mr-3 bg-slate-100 border border-slate-200 w-6 h-6 md:w-7 md:h-7 flex items-center justify-center rounded-full text-slate-600 shadow-sm">${huruf}</span><span class="text-xs md:text-sm font-medium text-slate-700 leading-snug">${formatMath(val)}</span></label>`; }); htmlOpsi += `</div>`; }
                ...
                if (q.tipe === 'BS') headers = ['Benar', 'Salah']; else if (q.tipe === 'TS') headers = ['Sesuai', 'Tidak Sesuai']; else if (q.tipe === 'SIFAT') headers = ['Sifat Komutatif', 'Sifat Asosiatif', 'Sifat Distributif'];
...
function colorizeAnswer(jawab, kunci, tipe) {
    ...
    if (jawab === kunci) return `<span style="color:#2563eb; font-weight:bold;">${formatMath(jawab)}</span>`; 
    let jArr = jawab.split(/, | \| /); let kArr = (kunci||"").split(/, | \| /);
    if (jArr.length > 1 && ['BS', 'TS', 'SIFAT', 'JODOH', 'PGK', 'ISIAN', 'ESAI'].includes(tipe)) {
        let res = []; jArr.forEach((j) => { if (kArr.includes(j)) res.push(`<span style="color:#2563eb; font-weight:bold;">${formatMath(j)}</span>`); else res.push(`<span style="color:#dc2626; font-weight:bold;">${formatMath(j)}</span>`); });
        return res.join('<br>');
...
function getFullAnswerText(q, rawAnswer) {
    ...
    if(q.tipe === 'JODOH') { let ansArr = rawAnswer.split(','); let texts = []; ansArr.forEach(a => { let numberPart = a.replace(/[a-zA-Z]/g, ''); let letterPart = a.replace(/[0-9]/g, ''); let idx = abjad.indexOf(letterPart); if(idx !== -1 && opsiArray[idx]) texts.push(`No.${numberPart} -> ${opsiArray[idx]}`); else texts.push(a); }); return texts.join(' | '); }
    if(q.tipe === 'BS' || q.tipe === 'TS' || q.tipe === 'SIFAT') { 
        let headers = []; if (q.tipe === 'BS') headers = ['Benar', 'Salah']; else if (q.tipe === 'TS') headers = ['Sesuai', 'Tidak Sesuai']; else if (q.tipe === 'SIFAT') headers = ['Sifat Komutatif', 'Sifat Asosiatif', 'Sifat Distributif'];
        let ansArr = rawAnswer.split(','); let texts = []; 
        ansArr.forEach((a, i) => { let idx = abjad.indexOf(a); if (q.tipe === 'BS' || q.tipe === 'TS') { if(a === 'B' && headers.length === 2 && idx === 1) { } else if(a === 'B' && !['A','B','C','D'].includes(a)) { idx = 0; } else if(a === 'S' && !['A','B','C','D'].includes(a)) { idx = 1; } } if(idx !== -1 && headers[idx]) texts.push(`No.${i+1}:${headers[idx]}`); else texts.push(`No.${i+1}:-`); }); return texts.join(', '); 
    } return rawAnswer;
}
...
async function submitUjian(showConfirm = true, isForceCurang = false) {
    ...
    if(!isGformOnly) {
        cbtQuestions.forEach((q, index) => {
            ...
            else if (q.tipe === 'PG') { totalSkorMaksimal += bobot; let kunciBersih = (q.kunci || "").replace(/\s/g, '').toLowerCase(); let ansBersih = (ans || "").replace(/\s/g, '').toLowerCase(); if(ansBersih && ansBersih === kunciBersih) { status = 'Benar'; poin = bobot; totalSkorDiperoleh += bobot; benar++; } else { salah++; } } 
            else if (q.tipe === 'PGK') { totalSkorMaksimal += bobot; let kunciArr = (q.kunci||"").replace(/\s/g, '').toLowerCase().split(',').filter(x=>x); let ansArr = (ans||"").replace(/\s/g, '').toLowerCase().split(',').filter(x=>x); if(kunciArr.length > 0) { let betul = 0; ansArr.forEach(a => { if(kunciArr.includes(a)) betul++; }); if(betul === kunciArr.length && ansArr.length === kunciArr.length) { status = 'Benar'; poin = bobot; totalSkorDiperoleh += bobot; benar++; } else if(betul > 0) { status = `Sebagian Benar (${betul}/${kunciArr.length})`; poin = (betul / kunciArr.length) * bobot; totalSkorDiperoleh += poin; benar++; } else { salah++; } } else { salah++; } } 
            else if (q.tipe === 'BS' || q.tipe === 'TS' || q.tipe === 'SIFAT') { totalSkorMaksimal += bobot; let kArr = (q.kunci||"").replace(/\s/g, '').toUpperCase().split(',').filter(x=>x); if ((q.tipe === 'BS' || q.tipe === 'TS') && kArr.some(k => k === 'S')) { kArr = kArr.map(k => k === 'B' ? 'A' : (k === 'S' ? 'B' : k)); } let aArr = (ans||"").replace(/\s/g, '').toUpperCase().split(','); let correctCount = 0; let totalItems = kArr.length; for(let j=0; j<totalItems; j++) { if(aArr[j] === kArr[j] && aArr[j] !== '-' && aArr[j] !== "") { correctCount++; } } if(correctCount === totalItems && totalItems > 0) { status = 'Benar'; poin = bobot; totalSkorDiperoleh += bobot; benar++; } else if (correctCount > 0) { status = `Sebagian Benar (${correctCount}/${totalItems})`; poin = (correctCount / totalItems) * bobot; totalSkorDiperoleh += poin; benar++; } else { salah++; } }
            else if (q.tipe === 'JODOH') { totalSkorMaksimal += bobot; let kArr = (q.kunci||"").replace(/\s/g, '').toLowerCase().split(',').filter(x=>x); let aArr = (ans||"").replace(/\s/g, '').toLowerCase().split(',').filter(x=>x); let correctCount = 0; let totalItems = kArr.length; kArr.forEach(k => { if(aArr.includes(k)) correctCount++; }); if(correctCount === totalItems && totalItems > 0) { status = 'Benar'; poin = bobot; totalSkorDiperoleh += bobot; benar++; } else if (correctCount > 0) { status = `Sebagian Benar (${correctCount}/${totalItems})`; poin = (correctCount / totalItems) * bobot; totalSkorDiperoleh += poin; benar++; } else { salah++; } }
            else if (q.tipe === 'ISIAN' || q.tipe === 'ESAI') { let kWords = (q.kunci || "").toLowerCase().match(/[a-z0-9]+/gi) || []; let aWords = (ans || "").toLowerCase().match(/[a-z0-9]+/gi) || []; if (kWords.length > 0) { totalSkorMaksimal += bobot; let matchWords = 0; let aWordsUnique = [...new Set(aWords)]; kWords.forEach(kw => { if(aWordsUnique.includes(kw)) matchWords++; }); if(matchWords === kWords.length) { status = 'Benar'; poin = bobot; totalSkorDiperoleh += bobot; benar++; } else if(matchWords > 0) { status = `Sebagian Benar (${matchWords}/${kWords.length} kata)`; poin = (matchWords / kWords.length) * bobot; totalSkorDiperoleh += poin; benar++; } else { salah++; } } else { status = 'Menunggu Koreksi Guru'; poin = 0; } }
    ...
    if (start) { 
        ...
        durasiText = `${dMins}m ${dSecs}s | ${terjawab}/${totalSoal} Soal`; 
    }
    ...
    if (showConfirm && !isForceCurang) {
        let kosong = cbtQuestions.filter((q, i) => { let a = cbtAnswers[i].ans; if(q.tipe === 'BS' || q.tipe === 'TS' || q.tipe === 'SIFAT') return a === "" || a.indexOf('-') !== -1; if(q.tipe === 'JODOH') { let h = (q.kunci||"").split(',').length; return a.split(',').length < h; } return a === ""; }).length;
...
async function resetSiswa(nama, mapel) { Swal.fire({ title: 'Buka Akses Ujian?', text: `Akses ujian ${nama} akan direset agar bisa login kembali tanpa menghapus jawabannya.`, icon: 'warning', showCancelButton: true, confirmButtonText: 'Ya, Buka Akses!' }).then(async (result) => { if (result.isConfirmed) { Swal.fire({title: 'Membuka akses...', didOpen: () => Swal.showLoading()}); await fetch(API + '/admin/reset-siswa', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({student_name: nama, mapel: mapel}) }); loadStats(); Swal.fire('Berhasil', 'Akses telah dibuka. Silakan suruh siswa login kembali.', 'success'); } }); }
async function usirHantu(id) { Swal.fire({ title: 'Hapus dari Layar?', text: 'Nama siswa akan dihapus dari layar ini.', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', confirmButtonText: 'Ya, Bersihkan!' }).then(async (result) => { if (result.isConfirmed) { await fetch(API + '/admin/remove-activity/' + id, { method: 'DELETE' }); loadStats(); } }); }
async function clearMonitoring() { Swal.fire({title: 'Bersihkan Layar?', text: 'Seluruh riwayat akan dihapus dari layar ini.', icon: 'warning', showCancelButton: true}).then(async (r) => { if(r.isConfirmed) { await fetch(API + '/admin/clear-monitoring', {method:'DELETE'}); loadStats(); Swal.fire('Bersih!', '', 'success'); } }); }
...
function renderActivityTable() {
    ...
    const renderTableGroup = (title, icon, titleColorClass, dataArray, rowColorClass, textClass, badgeClass, isWorking) => {
        ...
        let rows = dataArray.map(a => {
            ...
            let skorTampil = (isWorking && a.status === 'Mengerjakan' && a.score) ? `<span class="text-blue-600 font-bold"><i class="fa fa-chart-line"></i> Live: ${nilaiTampil}</span>` : ((isWorking && a.status === 'Mengerjakan') ? `<span class="text-orange-500 font-bold text-[9px] md:text-[10px]">-</span>` : nilaiTampil);
            let durasiFinal = '<span class="text-slate-400">-</span>'; let jamUpdate = a.last_seen || '-';
            ...
            let hantuBtn = (isWorking && activeUser && activeUser.role.toLowerCase() === 'admin') ? `<button onclick="usirHantu(${a.id})" class="ml-2 bg-red-100 text-red-600 hover:bg-red-500 hover:text-white px-2 py-1 rounded shadow-sm text-[8px]" title="Hapus Siswa (Khusus Admin)"><i class="fa fa-times"></i></button>` : '';
    ...
    if(!html) html = `<div class="text-center p-8 text-slate-400 font-bold bg-white rounded-xl border border-slate-200 border-dashed"><i class="fa fa-info-circle text-3xl mb-2 text-slate-300"></i><br>Belum ada aktivitas siswa.</div>`;
...
function renderPublicTable() {
    ...
    let html = filtered.map(a => {
        ...
        let skorMentah = a.score !== null && a.score !== undefined ? String(a.score) : '-'; let nilaiTampil = skorMentah.includes('|') ? skorMentah.split('|')[0].trim() : skorMentah;
        let waktuProgress = '<span class="text-slate-400">-</span>';
        if (a.last_seen && a.last_seen.includes('(')) {
    ...
    document.getElementById('public-table-body').innerHTML = html || `<tr><td colspan="5" class="text-center p-4 text-slate-400">Belum ada data aktivitas ujian.</td></tr>`;
}
...
async function loadNilai() { const res = await fetch(API + '/admin/results' + getAuthParams()); const data = await res.json(); window.allResultsData = data; if(document.getElementById('filter-kelas-nilai').options.length === 1) { let kelasSet = new Set(); let mapelSet = new Set(); window.allResultsData.forEach(r => { if(r.kelas && r.kelas !== '-') kelasSet.add(r.kelas); if(r.mapel) mapelSet.add(r.mapel); }); const fKelas = document.getElementById('filter-kelas-nilai'); kelasSet.forEach(k => { fKelas.add(new Option(k, k)); }); const fMapel = document.getElementById('filter-mapel-nilai'); mapelSet.forEach(m => { fMapel.add(new Option(m, m)); }); } renderNilaiTable(); }
function renderNilaiTable() { const selKelas = document.getElementById('filter-kelas-nilai').value; const selMapel = document.getElementById('filter-mapel-nilai').value; window.filteredResultsData = window.allResultsData.filter(r => { return (selKelas === "" || r.kelas === selKelas) && (selMapel === "" || r.mapel === selMapel); }); document.getElementById('nilai-body').innerHTML = window.filteredResultsData.map(n => `<tr><td class="p-2 md:p-3 font-medium whitespace-normal min-w-[120px] md:min-w-[150px] leading-snug">${n.student_name} <br><span class="text-[8px] md:text-[9px] text-slate-400">Kelas: ${n.kelas||'-'}</span></td><td>${n.mapel}</td><td class="text-emerald-600 font-bold">${n.benar || 0}</td><td class="text-red-500 font-bold">${n.salah || 0}</td><td class="font-black text-xs md:text-sm">${n.nilai}</td><td><button onclick='lihatDetail(${JSON.stringify(n.detail_jawaban || "[]").replace(/'/g, "'")})' class="bg-blue-100 text-blue-700 px-2 py-1 md:px-3 md:py-1 rounded text-[9px] md:text-[10px] font-bold">Detail</button></td></tr>`).join(''); }
...
function exportExcelDetail() { if(!window.filteredResultsData || window.filteredResultsData.length === 0) return Swal.fire('Kosong', 'Belum ada data nilai pada filter ini', 'warning'); let maxQ = 0; window.filteredResultsData.forEach(r => { try { let details = JSON.parse(r.detail_jawaban); if(details.length > maxQ) maxQ = details.length; } catch(e){} }); let tableHtml = `<table border="1"><thead><tr><th style="background-color: #f8fafc; font-weight: bold;">Nama Siswa</th><th style="background-color: #f8fafc; font-weight: bold;">Kelas</th><th style="background-color: #f8fafc; font-weight: bold;">Mapel</th><th style="background-color: #f8fafc; font-weight: bold;">Waktu Pengerjaan</th><th style="background-color: #f8fafc; font-weight: bold;">Benar</th><th style="background-color: #f8fafc; font-weight: bold;">Salah</th><th style="background-color: #f8fafc; font-weight: bold;">Nilai Akhir</th>`; for(let i=1; i<=maxQ; i++) { tableHtml += `<th style="background-color: #f8fafc; font-weight: bold; vertical-align: top;">Soal No.${i}</th>`; } tableHtml += `</tr></thead><tbody>`; window.filteredResultsData.forEach(r => { let details = []; try { details = JSON.parse(r.detail_jawaban); } catch(e){} let tgl = r.tanggal || ''; let durasi = '-'; if (tgl.includes('|')) durasi = tgl.split('|')[1]; tableHtml += `<tr><td style="vertical-align: top;">${r.student_name}</td><td style="vertical-align: top;">${r.kelas||'-'}</td><td style="vertical-align: top;">${r.mapel}</td><td style="vertical-align: top; font-weight: bold;">${durasi}</td><td style="vertical-align: top;">${r.benar}</td><td style="vertical-align: top;">${r.salah}</td><td style="font-weight: bold; vertical-align: top;">${r.nilai}</td>`; let detMap = {}; details.forEach(d => { detMap[d.no] = d; }); for(let i=1; i<=maxQ; i++) { let d = detMap[i]; if(d) { let coloredJawab = colorizeAnswer(d.jawab, d.kunci, d.tipe); tableHtml += `<td style="vertical-align: top;">${coloredJawab}</td>`; } else { tableHtml += `<td style="vertical-align: top;">-</td>`; } } tableHtml += `</tr>`; }); tableHtml += `</tbody></table>`; let blob = new Blob([tableHtml], {type: 'application/vnd.ms-excel'}); let a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'Hasil_Ujian_Lengkap_Berwarna.xls'; a.click(); }
...
async function hapusPaketSoal(examId) { Swal.fire({ title: `Hapus Paket ${examId}?`, text: "Seluruh soal di dalam paket ini akan terhapus secara permanen!", icon: "warning", showCancelButton: true, confirmButtonColor: "#dc2626", confirmButtonText: "Ya, Hapus Paket" }).then(async (r) => { if(r.isConfirmed) { Swal.fire({title: 'Menghapus...', didOpen: ()=>Swal.showLoading()}); await fetch(API + '/admin/delete-exam/' + encodeURIComponent(examId), {method: 'DELETE'}); loadBankSoal(); loadJadwal(); Swal.fire('Terhapus!', 'Paket soal berhasil dihapus.', 'success'); } }); }
async function hapusJadwal(id) { Swal.fire({title: 'Hapus Jadwal?', text: 'Soal tidak terhapus, hanya jadwal saja.', icon: 'warning', showCancelButton: true}).then(async (r) => { if(r.isConfirmed) { await fetch(API+'/admin/delete-schedule/'+id, {method:'DELETE'}); loadJadwal(); Swal.fire('Dihapus!', 'Jadwal berhasil dihapus.', 'success'); } }); }
function editJadwal(id) { const j = window.allSchedulesData.find(x => x.id === id); if(!j) return; let tglJamArr = j.tanggal ? j.tanggal.split('|') : []; let tgl = tglJamArr[0] || ''; let jam = tglJamArr[1] || ''; Swal.fire({ title: 'Edit Jadwal', html: `<div class="space-y-3 text-left"><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Mapel / Kode Soal</label><input id="e_j_mapel" class="w-full p-2 border rounded bg-slate-100 font-bold" value="${j.mapel}" readonly></div><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Tanggal Mulai</label><input type="date" id="e_j_tgl" class="w-full p-2 border rounded" value="${tgl}"></div><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Jam Mulai</label><input type="time" id="e_j_jam" class="w-full p-2 border rounded" value="${jam}"></div><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Durasi (Menit)</label><input type="number" id="e_j_durasi" class="w-full p-2 border rounded font-bold" value="${j.durasi}"></div><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Status Ujian</label><select id="e_j_status" class="w-full p-2 border rounded font-bold text-blue-700"><option value="Aktif" ${j.status==='Aktif'?'selected':''}>Aktif</option><option value="Ditutup" ${j.status!=='Aktif'?'selected':''}>Ditutup (Kunci Ujian)</option></select></div></div>`, showCancelButton: true, confirmButtonText: 'Simpan', confirmButtonColor: '#2563eb', preConfirm: () => { return { id: j.id, mapel: document.getElementById('e_j_mapel').value, tanggal: document.getElementById('e_j_tgl').value + '|' + document.getElementById('e_j_jam').value, durasi: document.getElementById('e_j_durasi').value, status: document.getElementById('e_j_status').value } } }).then(async (res) => { if(res.isConfirmed) { Swal.fire({title: 'Menyimpan...', didOpen: ()=>Swal.showLoading()}); await fetch(API+'/admin/update-schedule', {method: 'PUT', headers: {'Content-Type':'application/json'}, body: JSON.stringify(res.value)}); loadJadwal(); Swal.fire('Tersimpan!', 'Jadwal berhasil diperbarui.', 'success'); } }); }
async function saveJadwal() { const mapel = document.getElementById('j_mapel').value; const tanggal = document.getElementById('j_tgl').value; const jam = document.getElementById('j_jam').value; const durasi = document.getElementById('j_durasi').value; if(!mapel || !tanggal || !jam || !durasi) return Swal.fire('Oops','Semua kolom jadwal wajib diisi!','warning'); const tglJamDB = `${tanggal}|${jam}`; Swal.fire({ title: 'Menyimpan Jadwal...', didOpen: () => Swal.showLoading() }); try { const response = await fetch(API + '/admin/add-schedule', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ mapel, tanggal: tglJamDB, durasi }) }); const data = await response.json(); if (data.status === 'success') { Swal.fire('Sukses!', 'Jadwal ujian berhasil dibuat.', 'success'); loadJadwal(); } else { Swal.fire('Gagal Menyimpan!', data.message || 'Error Database.', 'error'); } } catch(e) { Swal.fire('Error Server', 'Terjadi kesalahan.', 'error'); } }
async function loadJadwal() { const res = await fetch(API + '/admin/schedules' + getAuthParams()); const data = await res.json(); window.allSchedulesData = data; document.getElementById('list-jadwal').innerHTML = data.map(j => { let tglJamArr = j.tanggal ? j.tanggal.split('|') : []; let tglTampil = tglJamArr[0] || j.tanggal; let jamTampil = tglJamArr[1] ? ` • Jam ${tglJamArr[1]}` : ''; let statusBadge = j.status === 'Aktif' ? '<span class="bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded text-[8px] font-bold">Aktif</span>' : '<span class="bg-red-100 text-red-700 px-1.5 py-0.5 rounded text-[8px] font-bold">Ditutup</span>'; return `<div class="bg-white p-3 md:p-4 rounded-xl border border-l-4 border-blue-500 shadow-sm flex justify-between items-center relative"><div class="flex-1"><div class="flex items-center gap-2 mb-1"><h4 class="font-bold text-[10px] md:text-sm text-blue-900">${j.mapel}</h4> ${statusBadge}</div><p class="text-[8px] md:text-[10px] text-slate-400"><i class="fa fa-calendar-alt"></i> ${tglTampil}${jamTampil} • <i class="fa fa-clock"></i> ${j.durasi} Menit</p></div><div class="text-center px-3 md:px-4 border-l border-slate-100"><p class="text-[7px] md:text-[8px] font-bold text-slate-400">PIN UJIAN</p><p class="text-base md:text-lg font-black text-blue-600 font-mono tracking-widest">${j.pin}</p></div><div class="flex flex-col gap-1.5 pl-2 border-l border-slate-100"><button onclick="editJadwal(${j.id})" class="bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white px-2 py-1.5 rounded transition shadow-sm text-[10px] md:text-xs" title="Edit Jadwal"><i class="fa fa-edit"></i></button><button onclick="hapusJadwal(${j.id})" class="bg-red-50 text-red-600 hover:bg-red-600 hover:text-white px-2 py-1.5 rounded transition shadow-sm text-[10px] md:text-xs" title="Hapus Jadwal"><i class="fa fa-trash"></i></button></div></div>`; }).join(''); const resExams = await fetch(API + '/admin/available-exams' + getAuthParams()); const exams = await resExams.json(); const selectMapel = document.getElementById('j_mapel'); selectMapel.innerHTML = '<option value="">-- Pilih Kode Soal --</option>' + exams.map(e => `<option value="${e}">${e}</option>`).join(''); }
...
let questionCount = 1;
function tambahBarisSoal() { questionCount++; const container = document.getElementById('bulk-questions-container'); const html = `<div class="question-item bg-slate-50 p-3 md:p-4 rounded-2xl border-2 border-slate-200 relative mt-4 md:mt-6" data-no="${questionCount}"><div class="absolute -left-3 -top-3 w-6 h-6 md:w-8 md:h-8 bg-slate-800 text-white rounded-full flex items-center justify-center font-black shadow-lg text-xs md:text-sm">${questionCount}</div><button onclick="this.parentElement.remove()" class="absolute -right-2 -top-2 w-6 h-6 bg-red-500 text-white rounded-full text-[10px] md:text-xs shadow-md"><i class="fa fa-times"></i></button><div class="grid grid-cols-1 gap-2 md:gap-3"><div class="grid grid-cols-1 md:grid-cols-3 gap-2"><div><label class="text-[9px] md:text-[10px] font-bold text-slate-500">TIPE SOAL</label><select class="q-tipe w-full p-2 border rounded-lg text-[10px] md:text-xs outline-none focus:border-blue-500"><option value="PG">1 - Pilihan Ganda Biasa</option><option value="PGK">2 - PG Kompleks (Centang)</option><option value="JODOH">3 - Menjodohkan</option><option value="ISIAN">4 - Isian Singkat</option><option value="ESAI">5 - Uraian (Esai)</option><option value="BS">7 - Benar/Salah (Tabel Asli)</option><option value="TS" class="font-bold text-emerald-600">9 - TS (Tabel Sesuai)</option><option value="SIFAT">10 - SIFAT (Komutatif dll)</option><option value="GFORM" class="font-bold text-purple-600">8 - Link G-Form</option></select></div><div><label class="text-[9px] md:text-[10px] font-bold text-slate-500">KUNCI JAWABAN</label><input type="text" class="q-kunci w-full p-2 border rounded-lg text-[10px] md:text-xs" placeholder="Misal: A, atau A,C"></div><div><label class="text-[9px] md:text-[10px] font-bold text-blue-600">SKOR BOBOT</label><input type="number" class="q-skor w-full p-2 border border-blue-300 bg-blue-50 rounded-lg text-[10px] md:text-xs font-bold" value="1"></div></div><div><label class="text-[9px] md:text-[10px] font-bold text-blue-500"><i class="fa fa-image"></i> LINK GAMBAR DRIVE</label><input type="text" class="q-image w-full p-2 border rounded-lg text-[10px] md:text-xs" placeholder="http://..."></div><div><label class="text-[9px] md:text-[10px] font-bold text-slate-500">PERTANYAAN (Awali dgn angka misal "1. Teks" untuk B-S/TS/Jodoh)</label><textarea class="q-tanya w-full p-2 border rounded-lg text-xs md:text-sm h-12 md:h-16"></textarea></div><div class="q-area-opsi"><label class="text-[9px] md:text-[10px] font-bold text-orange-600">OPSI (Gunakan ||| sebagai pemisah)</label><input type="text" class="q-opsi w-full p-2 border border-orange-200 rounded-lg text-[10px] md:text-xs" placeholder="B/S: Pernyataan 1 ||| Pernyataan 2. TS: Sesuai ||| Tidak Sesuai"></div></div></div>`; container.insertAdjacentHTML('beforeend', html); window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' }); }
...
async function loadBankSoal() { const res = await fetch(API + '/admin/questions' + getAuthParams()); const data = await res.json(); if (data.length === 0) { document.getElementById('banksoal-container').innerHTML = '<div class="text-center p-8 text-slate-400 bg-white rounded-xl border border-slate-200 shadow-sm"><i class="fa fa-box-open text-3xl mb-2"></i><br>Belum ada soal.</div>'; return; } const groups = {}; data.forEach(q => { if(!groups[q.exam_id]) groups[q.exam_id] = []; groups[q.exam_id].push(q); }); let html = ''; for (const [examId, questions] of Object.entries(groups)) { let tableRows = questions.map(q => { let det = q.tipe === 'GFORM' ? `<span class="text-purple-600 font-bold">Link G-Form: ${q.tanya || q.gform_url}</span>` : (q.tanya ? formatMath(q.tanya).substring(0,80)+'...' : '-'); det += `<br><span class="text-[9px] md:text-[10px] font-bold text-emerald-600 mt-1 md:mt-2 inline-block p-1 bg-emerald-50 border border-emerald-200 rounded"><i class="fa fa-key"></i> KUNCI: <b class="text-emerald-900">${formatMath(q.kunci) || '-'}</b></span> <span class="text-[9px] md:text-[10px] font-bold text-blue-600 mt-1 md:mt-2 inline-block p-1 bg-blue-50 border border-blue-200 rounded ml-1"><i class="fa fa-star"></i> SKOR: <b class="text-blue-900">${q.skor || 1}</b></span>`; return `<tr class="hover:bg-slate-50 transition"><td class="align-top pt-3 md:pt-4 w-16 md:w-20 px-2 md:px-4 border-b border-slate-100"><span class="px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-[8px] md:text-[9px] font-black shadow-sm">${q.tipe}</span></td><td class="text-[10px] md:text-xs text-slate-700 py-3 md:py-4 px-2 border-b border-slate-100 whitespace-normal min-w-[200px] md:min-w-[250px] leading-relaxed">${det}</td><td class="p-2 md:p-4 align-top text-right w-16 md:w-24 border-b border-slate-100"><button onclick="hapusSoal(${q.id})" class="text-red-500 hover:text-white hover:bg-red-500 border border-red-200 px-2 py-1 md:px-3 md:py-2 rounded-lg transition shadow-sm" title="Hapus Soal"><i class="fa fa-trash"></i></button></td></tr>`; }).join(''); html += `<div class="mb-3 md:mb-4 bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden"><div class="w-full bg-slate-50 p-3 md:p-4 font-bold text-slate-700 flex justify-between items-center hover:bg-blue-50 transition"><button onclick="document.getElementById('soal-${examId}').classList.toggle('hidden')" class="flex-1 flex items-center text-[10px] md:text-sm text-left"><i class="fa fa-folder-open text-blue-500 mr-2 md:mr-3 text-base md:text-lg"></i> ${examId} <span class="bg-blue-100 text-blue-700 px-2 py-0.5 md:py-1 rounded-full text-[8px] md:text-[10px] ml-2 md:ml-3 shadow-inner">${questions.length} Soal</span></button><div class="flex items-center gap-3"><button onclick="hapusPaketSoal('${examId.replace(/'/g, "\\'")}')" class="bg-red-100 text-red-600 hover:bg-red-600 hover:text-white px-2 md:px-3 py-1 md:py-1.5 rounded text-[9px] md:text-xs font-bold transition shadow-sm"><i class="fa fa-trash mr-1"></i> Hapus Paket</button><i class="fa fa-chevron-down text-slate-400 text-xs md:text-sm cursor-pointer p-2" onclick="document.getElementById('soal-${examId}').classList.toggle('hidden')"></i></div></div><div id="soal-${examId}" class="hidden overflow-x-auto border-t border-slate-100"><table class="w-full text-left text-xs whitespace-nowrap"><tbody class="divide-y divide-slate-100">${tableRows}</tbody></table></div></div>`; } document.getElementById('banksoal-container').innerHTML = html; }
...
async function loadUsers() { const res = await fetch(API + '/admin/users'); const data = await res.json(); document.getElementById('user-body').innerHTML = data.map(u => `<tr><td class="p-2 md:p-3 font-medium whitespace-normal min-w-[120px] md:min-w-[150px] leading-snug">${u.name}</td><td>${u.username}</td><td>${u.kelas || u.mapel || '-'}</td><td><span class="px-2 py-1 bg-slate-100 rounded text-[9px] md:text-[10px]">${u.role}</span></td><td class="text-right p-2 md:p-3 min-w-[100px]"><button onclick='editUser(${JSON.stringify(u).replace(/'/g, "'")})' class="bg-blue-100 text-blue-600 hover:bg-blue-600 hover:text-white px-2 py-1 rounded shadow-sm transition mr-1"><i class="fa fa-edit"></i> Edit</button><button onclick="hapusUser('${u.username}')" class="bg-red-100 text-red-600 hover:bg-red-600 hover:text-white px-2 py-1 rounded shadow-sm transition"><i class="fa fa-trash"></i></button></td></tr>`).join(''); }

function tambahUserManual() { Swal.fire({ title: 'Tambah Pengguna', html: `<div class="space-y-3 text-left"><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Nama Lengkap</label><input id="a_name" class="w-full p-2 border rounded" placeholder="Nama Lengkap"></div><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Username</label><input id="a_user" class="w-full p-2 border rounded" placeholder="Username (tanpa spasi)"></div><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Password</label><input id="a_pass" class="w-full p-2 border rounded" placeholder="Password"></div><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Akses Role</label><select id="a_role" class="w-full p-2 border rounded"><option value="siswa">Siswa</option><option value="guru">Guru</option><option value="admin">Admin</option></select></div><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Kelas (Khusus Siswa)</label><input id="a_kelas" class="w-full p-2 border rounded" placeholder="Contoh: VIII-A"></div><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Mapel (Khusus Guru)</label><input id="a_mapel" class="w-full p-2 border rounded" placeholder="Contoh: IPA, MTK"></div></div>`, showCancelButton: true, confirmButtonText: 'Simpan', confirmButtonColor: '#059669', preConfirm: () => { return { name: document.getElementById('a_name').value, username: document.getElementById('a_user').value, password: document.getElementById('a_pass').value, role: document.getElementById('a_role').value, kelas: document.getElementById('a_kelas').value, mapel: document.getElementById('a_mapel').value } } }).then(async (res) => { if(res.isConfirmed) { if(!res.value.username || !res.value.password) return Swal.fire('Error', 'Username dan Password Wajib Diisi!', 'error'); Swal.fire({title: 'Menyimpan...', didOpen: ()=>Swal.showLoading()}); await fetch(API+'/admin/add-user', {method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(res.value)}); loadUsers(); Swal.fire('Tersimpan!', 'Pengguna baru berhasil ditambahkan.', 'success'); } }); }
function editUser(u) { Swal.fire({ title: 'Edit Pengguna', html: `<div class="space-y-3 text-left"><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Nama Lengkap</label><input id="e_name" class="w-full p-2 border rounded" value="${u.name}"></div><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Username</label><input id="e_user" class="w-full p-2 border rounded" value="${u.username}"></div><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Password</label><input id="e_pass" class="w-full p-2 border rounded" value="${u.password}"></div><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Akses Role</label><select id="e_role" class="w-full p-2 border rounded"><option value="siswa" ${u.role==='siswa'?'selected':''}>Siswa</option><option value="guru" ${u.role==='guru'?'selected':''}>Guru</option></select></div><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Kelas (Khusus Siswa)</label><input id="e_kelas" class="w-full p-2 border rounded" value="${u.kelas||''}"></div><div><label class="text-[10px] md:text-xs font-bold text-slate-500">Mapel (Khusus Guru)</label><input id="e_mapel" class="w-full p-2 border rounded" value="${u.mapel||''}"></div></div>`, showCancelButton: true, confirmButtonText: 'Simpan', confirmButtonColor: '#2563eb', preConfirm: () => { return { old_username: u.username, name: document.getElementById('e_name').value, username: document.getElementById('e_user').value, password: document.getElementById('e_pass').value, role: document.getElementById('e_role').value, kelas: document.getElementById('e_kelas').value, mapel: document.getElementById('e_mapel').value } } }).then(async (res) => { if(res.isConfirmed) { Swal.fire({title: 'Menyimpan...', didOpen: ()=>Swal.showLoading()}); await fetch(API+'/admin/update-user', {method: 'PUT', headers: {'Content-Type':'application/json'}, body: JSON.stringify(res.value)}); loadUsers(); Swal.fire('Tersimpan!', 'Data berhasil diperbarui.', 'success'); } }); }
async function hapusUser(usr) { Swal.fire({title: 'Hapus User?', text: "Data tidak bisa dikembalikan!", icon: 'warning', showCancelButton: true}).then(async (r) => { if(r.isConfirmed) { await fetch(API+'/admin/delete-user/'+usr, {method:'DELETE'}); loadUsers(); Swal.fire('Dihapus!', 'Pengguna telah dihapus.', 'success'); } }); }
