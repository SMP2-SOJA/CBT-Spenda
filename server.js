...
// PENGHAPUSAN AKTIVITAS MONITORING SAAT GANTI JAM
...
// VALIDASI JAM DAN TANGGAL PADA SAAT CEK PIN
app.post('/api/siswa/cek-pin', async (req, res) => { 
    ...
    if(row) { 
        // Pengecekan Tanggal dan Jam Ujian (Digabung pada parameter tanggal dengan pemisah |)
        let scheduleDate = row.tanggal; let scheduleTime = null;
        if(row.tanggal && row.tanggal.includes('|')) {
            let parts = row.tanggal.split('|'); scheduleDate = parts[0]; scheduleTime = parts[1];
        }
        ...
        if (scheduleTime && req.body.client_time < scheduleTime) {
