...
// ==========================================
const supabaseUrl = 'URL_SUPABASE_MAS_RAHMAT';
const supabaseKey = 'PUBLISH_KEY_SUPABASE_MAS_RAHMAT';
...
app.post('/api/admin/add-soal', upload.single('media'), async (req, res) => {
    const { exam_id, tipe, tanya, opsi, kunci, gform_url } = req.body;
    const media_path = req.file ? req.file.path.replace(/\\/g, '/') : null;
    await supabase.from('questions').insert([{ exam_id, tipe, tanya, opsi_json: opsi, kunci, media_path, gform_url }]);
    res.json({status:"success"});
});

app.post('/api/admin/import-word', upload.single('file_word'), async (req, res) => {
    try {
        const result = await mammoth.convertToHtml({path: req.file.path}); const rows = result.value.match(/<tr[^>]*>([\s\S]*?)<\/tr>/gi);
        if (!rows || rows.length < 2) return res.status(400).json({status: "error", message: "Tabel tidak ditemukan"});
        let insertData = [];
        for(let i = 1; i < rows.length; i++) {
            ...
            if (cells && cells.length >= 2) {
                ...
                if (textCells[1]) insertData.push({ exam_id: req.body.exam_id, tipe: (textCells[0] || 'PG').toUpperCase(), tanya: textCells[1], opsi_json: opsi.join(','), kunci: textCells[6] || '' });
            }
        }
        if(insertData.length > 0) await supabase.from('questions').insert(insertData);
        res.json({status: 'success', message: `${insertData.length} Soal diekstrak!`});
    } catch (e) { res.status(500).json({status: 'error', message: 'Gagal'}); }
});
...
app.post('/api/admin/import-soal', upload.single('file_excel'), async (req, res) => {
    try {
        ...
        if(insertData.length > 0) await supabase.from('questions').insert(insertData);
        res.json({ status: "success", message: `${insertData.length} Soal di-generate!` });
...
app.get('/api/admin/stats', async (req, res) => {
    ...
    (acts || []).forEach(a => { if (a.status === 'Mengerjakan') stats.sedang_kerja++; else if (a.status === 'Selesai') stats.selesai++; else if (a.status === 'Curang') stats.curang++; });
    res.json(stats);
});
...
app.post('/api/admin/import-users', upload.single('file_excel'), async (req, res) => {
    try {
        ...
        let insertData = excelData.map(row => ({ name: row.Nama, username: row.Username, password: row.Password, role: row.Role || 'siswa', kelas: row.Kelas, mapel: row.Mapel || '' }));
        if(insertData.length > 0) await supabase.from('users').insert(insertData);
        res.json({ status: "success", message: `${insertData.length} Peserta diimport!` });
    } catch(e) { res.status(500).json({status: "error"}); }
});
...
app.post('/api/siswa/cek-pin', async (req, res) => {
    ...
    if(row) {
        await supabase.from('activity').insert([{ student_name, exam_name: row.mapel, status: 'Mengerjakan', last_seen: new Date().toLocaleTimeString('id-ID') }]);
        res.json({status: "success", exam: row});
    } else res.status(404).json({status: "error", message: "PIN Tidak Sah!"}); 
});
...
// =======================================================
// PERBAIKAN: API SUBMIT MENERIMA & MENYIMPAN SKOR DETAIL
// =======================================================
...
// Fitur Anti-Curang Level Server: Flag Siswa Curang
...
app.listen(3000, () => console.log("CBT PRO RUNNING"));