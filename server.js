...
app.post('/api/login', async (req, res) => { 
    ...
    if (data) {
        ...
        res.status(401).json({ status: "error", message: "Username/Password Salah!" }); 
    }
});
...
app.get('/api/admin/questions', async (req, res) => { 
    ...
    if (req.query.role === 'guru') {
        data = (data || []).filter(q => isAuthorizedMapel(req.query.mapel, q.exam_id)); 
    }
    res.json(data || []); 
});
...
app.get('/api/admin/available-exams', async (req, res) => { 
    ...
    if (req.query.role === 'guru') {
        exams = exams.filter(e => isAuthorizedMapel(req.query.mapel, e)); 
    }
    res.json(exams); 
});
...
app.delete('/api/admin/delete-soal/:id', async (req, res) => { 
    ...
    if (soal) { 
        ...
        if (count === 0) {
            await supabase.from('schedules').delete().eq('mapel', soal.exam_id); 
        }
        res.json({status: "success"}); 
    } else { 
...
app.delete('/api/admin/clear-questions', async (req, res) => { 
...
app.post('/api/admin/add-schedule', async (req, res) => { 
    ...
    if (error) return res.status(500).json({ status: "error", message: `Supabase Error: ${error.message}` }); 
...
app.get('/api/admin/schedules', async (req, res) => { 
    ...
    if (req.query.role === 'guru') {
        data = (data || []).filter(s => isAuthorizedMapel(req.query.mapel, s.mapel)); 
    }
    res.json(data || []); 
});
...
app.get('/api/admin/stats', async (req, res) => { 
    const stats = { total_siswa: 0, total_guru: 0 }; 
    
    // PEMISAHAN HITUNGAN GURU DAN SISWA YANG MUTLAK
...
app.get('/api/admin/recent-activity', async (req, res) => { 
    ...
    let actsWithKelas = (acts || []).map(a => { 
        let u = (users || []).find(x => x.name === a.student_name); 
        return { ...a, kelas: u ? u.kelas : '-' }; 
    });
...
app.delete('/api/admin/clear-monitoring', async (req, res) => { 
...
app.get('/api/admin/users', async (req, res) => { 
    const { data } = await supabase.from('users').select('*').neq('role', 'admin').neq('role', 'Admin').order('id', {ascending: false}); 
    res.json(data || []); 
});
...
app.put('/api/admin/update-user', async (req, res) => { 
    const { old_username, name, username, password, role, kelas, mapel } = req.body; 
    await supabase.from('users').update({ name, username, password, role, kelas, mapel }).eq('username', old_username); 
    res.json({status: "success"}); 
});
...
app.delete('/api/admin/clear-users', async (req, res) => { 
    await supabase.from('users').delete().neq('role', 'admin').neq('role', 'Admin'); 
    res.json({status: "success"}); 
});
...
app.get('/api/admin/results', async (req, res) => { 
    ...
    let { data: users } = await supabase.from('users').select('name, kelas');
    let resultsWithKelas = (results || []).map(r => { 
        let u = (users || []).find(x => x.name === r.student_name); 
        return { ...r, kelas: u ? u.kelas : '-' }; 
    });
    if (req.query.role === 'guru') {
...
app.post('/api/admin/import-soal', upload.single('file_excel'), async (req, res) => { 
    try { 
        ...
        res.status(500).json({status: "error", message: "Gagal memproses Excel. Pastikan kolom Skor sudah ada di Supabase."}); 
    } 
});
    ...
    try { 
        ...
        for(let i = 0; i < rows.length; i++) { 
            ...
            if (cells && cells.length >= 5) { 
                ...
                let opsi = []; 
                for(let j=5; j<=9; j++) { 
                    if(textCells[j] && textCells[j].trim() !== '') opsi.push(textCells[j].trim()); 
                } 
...
app.post('/api/siswa/cek-pin', async (req, res) => { 
    ...
    if(row) { 
        if (row.status !== 'Aktif') {
            return res.status(403).json({status: "error", message: "Sesi Ujian sudah ditutup oleh Guru!"});
        }
        
        let scheduleDate = row.tanggal; 
        ...
        if (scheduleDate && scheduleDate !== req.body.client_date) {
            return res.status(403).json({status: "error", message: "Ujian tidak dijadwalkan pada hari ini!"});
        }
        ...
        // PENGECEKAN WAKTU DAN AUTO-LOCK
        if (scheduleTime) {
            if (req.body.client_time < scheduleTime) {
                return res.status(403).json({status: "error", message: `Ujian belum dimulai! (Jadwal: ${scheduleTime})`});
            }
        ...
        if (existingActs && existingActs.length > 0) {
            ...
            if (act.status && act.status.includes('Terkunci')) {
                return res.status(403).json({status: "error", message: "Akses Terkunci! Laporkan ke Guru Mapel untuk dibuka kembali."});
            } else if (act.status === 'Selesai') {
                return res.status(403).json({status: "error", message: "Anda sudah menyelesaikan ujian ini."});
            }
...
app.post('/api/siswa/get-soal', async (req, res) => { 
    const { data } = await supabase.from('questions').select('id, tipe, tanya, opsi_json, kunci, media_path, gform_url, skor').eq('exam_id', req.body.exam_id).order('id', {ascending: true}); 
