...
app.post('/api/admin/import-word', upload.single('file_word'), async (req, res) => { try { const exam_id = req.body.exam_id; if(!exam_id) return res.status(400).json({status: "error", message: "KODE UJIAN harus diisi!"}); const result = await mammoth.convertToHtml({path: req.file.path}); const rows = result.value.match(/<tr[^>]*>([\s\S]*?)<\/tr>/gi); if (!rows) return res.status(400).json({status: "error", message: "Tabel tidak ditemukan!"}); let insertData = []; for(let i = 0; i < rows.length; i++) { const cells = rows[i].match(/<td[^>]*>([\s\S]*?)<\/td>/gi); if (cells && cells.length >= 5) { const textCells = cells.map(c => c.replace(/<\/p>/gi, '\n').replace(/<br\s*\/?>/gi, '\n').replace(/<[^>]+>/g, '').trim()); let jenisAngka = textCells[1] ? textCells[1].replace(/[^0-9]/g, '') : ""; if (!['1','2','3','4','5'].includes(jenisAngka)) continue; let tipe = 'PG'; if (jenisAngka === '2') tipe = 'PGK'; else if (jenisAngka === '3') tipe = 'JODOH'; else if (jenisAngka === '4') tipe = 'ISIAN'; else if (jenisAngka === '5') tipe = 'ESAI'; const tanya = textCells[3] || ""; const fileSoal = textCells[4] || ""; let opsi = []; for(let j=5; j<=9; j++) { if(textCells[j] && textCells[j].trim() !== '') opsi.push(textCells[j].trim()); } const kunci = textCells[10] || ""; if (tanya !== '') insertData.push({ exam_id, tipe, tanya, opsi_json: opsi.join('|||'), kunci: kunci.trim().toUpperCase(), gform_url: fileSoal, skor: 1 }); } } if(insertData.length > 0) { await supabase.from('questions').insert(insertData); res.json({status: 'success', message: `${insertData.length} Soal Bimasoft berhasil diekstrak!`}); } else { res.status(400).json({status: 'error', message: '0 Soal diekstrak.'}); } } catch (e) { res.status(500).json({status: 'error', message: 'Gagal ekstrak.'}); } });
